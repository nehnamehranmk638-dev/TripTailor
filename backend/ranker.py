def calculate_score(spot, category_weights, budget_per_day):
    ...
    if weight <= 0.0:
        return 0.0
    budget_fit = 1.0 if float(spot.cost_usd) <= budget_per_day else 0.0
...
def smart_replan(current_itinerary, all_spots, category_weights,
    ...
    for day_slots in current_itinerary.values():
        for spot in day_slots.values():
            if spot and spot.get("id") in locked_ids:
    ...
    for day_num, day_slots in current_itinerary.items():
        ...
        for slot in time_slots:
            ...
            if spot and spot.get("id") in locked_ids:
                ...
                continue
            if spot and spot.get("id") not in disliked_ids:
                new_itinerary[day_num][slot] = spot
                continue
            ...
            if slot_replacements:
                ...
                new_itinerary[day_num][slot] = {
                    ...
                    "cost_usd": float(chosen.cost_usd),
                    "rating": float(chosen.rating),
                    "description": chosen.description,
                    "duration_hours": float(chosen.duration_hours) if chosen.duration_hours else 2.0,
                    "latitude": float(chosen.latitude) if chosen.latitude else None,
                    "longitude": float(chosen.longitude) if chosen.longitude else None
                }
                used_ids.add(chosen.id)
